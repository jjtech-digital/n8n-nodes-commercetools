...
function isMainUpdateOp(op: ParsedOperation): boolean {
    ...
    return op.method === 'POST' && op.requiresId
        && op.bodyFields.some((f) => f.name === 'actions');
}
...
export function generateOperationProperties(
    ...
    for (const folder of folders) {
        ...
        const topLevelOps = operations.filter(
            (op) => op.folder === folder && !op.isUpdateAction,
        );
...
export function generateVersionField(
    ...
    for (const folder of folders) {
        ...
        const opsNeedingVersion = operations
            .filter((op) => op.folder === folder && !op.isUpdateAction
...
export function generateActionsUiField(
    ...
    for (const folder of folders) {
        ...
        const updateActions = operations.filter(
            (op) => op.folder === folder && op.isUpdateAction,
        );
        ...
        const optionGroups = updateActions.map((op) => {
            ...
            console.log(`OP: ${op.name} | actionBodyFields:`, fields.map((f) => `${f.name}(${f.type})`));
    ...
    for (const folder of folders) {
        ...
        for (const op of eligibleOps) {
            ...
            const cleanParams = [...new Set(
                op.queryParams.filter((p) => p && !p.startsWith('/') && p.trim().length > 0),
            )];
