...
export function generateSubscriptionProperties(
	...
	const messageResources = new Set(
		parseEnumValues(subSrc, 'MessageSubscriptionResourceTypeIdValues'),
	);
	const changeResources = new Set(
		parseEnumValues(subSrc, 'ChangeSubscriptionResourceTypeIdValues'),
	);
	...
	if (allowedResources) {
		...
		for (const r of allowedResources) {
			if (!messageResources.has(r) && !changeResources.has(r) && !eventResources.has(r)) {
				console.warn(
					`  ⚠️  RESOURCES_TO_GENERATE: "${r}" is not a known CT resourceTypeId — check spelling`,
				);
			}
		}
...
function writeOutput(
	events: EventEntry[],
	outputPath: string,
	resourcesToInclude: string[] | null,
): void {
	...
	for (const [resourceTypeId, entries] of [...byResource.entries()].sort(([a], [b]) =>
		a.localeCompare(b),
	)) {
