...
export function parseAttributeValue(value: string, type: string): string | number | boolean {
	try {
		...
		throw new Error(
			`Failed to parse value as ${type}: ${(error as Error)?.message || String(error)}`,
		);
	}
}
			...
			if (
				...
				if (Array.isArray(fieldArray)) {
					for (const fieldItem of fieldArray) {
						if (typeof fieldItem === 'object' && 'name' in fieldItem && 'value' in fieldItem) {
...
export function handleSetSearchKeywords(action: IDataObject): IDataObject {
	if (action?.action === 'setSearchKeywords') {
		...
		for (const key of keys) {
			searchKeywordsObj[key] = ((action.searchKeywords as IDataObject)[key] as IDataObject)
				?.keyword as IDataObject[];
			const processedTokenizer = (searchKeywordsObj[key] as IDataObject[]).map(
				(keywordObj: IDataObject) => {
					const suggestTokenizer = (keywordObj?.suggestTokenizer as IDataObject)
						?.tokenizer as string[];
					return {
						...keywordObj,
...
export function handleAddVariant(action: IDataObject, localized: IDataObject): IDataObject {
	if (action?.action === 'addVariant') {
		...
		const attributesInput = (((localized.attributes as IDataObject) ?? {})?.attribute ??
			[]) as IDataObject[];
		const formattedAttributes = attributesInput
			.filter(
				(attribute) => attribute?.name && attribute?.value !== undefined && attribute?.value !== '',
			)
			.map((attribute) => {
				...
				return {
					...
					value: parseAttributeValue(
						...
						attributeWithValueType?.valueType ?? 'string',
					),
				};
			});
		...
		const formattedAssets = assetsInput
			.map((asset) => {
				...
				const formattedSources = sources
					.filter(
						(source) => typeof source?.uri === 'string' && (source.uri as string).trim() !== '',
					)
					.map((source) => {
						...
						const sourceDimensions = ((source.dimensions as IDataObject)?.size ??
							{}) as IDataObject;
...
export function handleSetAssetSources(action: IDataObject): IDataObject {
	if (action?.action === 'setAssetSources') {
		const flatSources =
			action.sources && typeof action.sources === 'object' && 'source' in action.sources
 ...
 */
export function handleChangeAssetOrder(
	...
	if (action?.action === 'changeAssetOrder') {
		...
		const secondaryOrder =
			typeof action.assetOrderSecond === 'string' ? action.assetOrderSecond.trim() : '';
...
export function handleAddAsset(action: IDataObject, localized: IDataObject): IDataObject {
	if (action?.action === 'addAsset') {
		...
		const assetSourcesFromAsset =
			(assetDraft.sources as IDataObject | IDataObject[] | undefined) ?? undefined;
